...
import java.io.*;		//DONE added this line on own decision.
...
class UserHandler implements Runnable
{
			...
			Logger parentLogger)
	{
		...
		if (in != null)
		{
			...
			/*
			 * DONE Création du BufferedReader sur un InputStreamReader à partir
			 * du flux d'entrée en provenance de l'utilisateur
			 */
			// userInBR = DONE Complete ...
		...
		if (out != null)
		{
			...
			/*
			 * DONE Création du PrintWriter sur le flux de sortie vers le
			 * serveur (en mode autoflush)
			 */
			// serverOutPW = DONE Complete ...
			serverOutPW = new PrintWriter(out, true); //TRUE pour activer l'autoFlush
		}
		else
		{
	...
	public void run()
	{
		...
		while (commonRun.booleanValue())
		{
			...
			{
				logger.severe("UserHandler: IOexception while reading input.");
				break;
			}
			...
			{
				/*
				 * DONE Envoi du texte au serveur grâce au serverOutPW et
				...
				if(serverOutPW.checkError()){
					logger.warning("UserHandler: sending message to server failed.");
					break;
				}
