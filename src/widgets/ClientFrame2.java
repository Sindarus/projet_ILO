...
public class ClientFrame2 extends AbstractClientFrame
{
	...
	protected void writeMessage(Message message) throws BadLocationException
	{
		...
		if (message.hasAuthor() && (message.getAuthor().length() > 0))
		{
			/*
	...
	public void run()
	{
		...
		try {
			...
			logger.warning("ClientFrame2 : IOException while creating inOS");
		}
		...
		while (commonRun.booleanValue())
		{
			...
			try
			{
				...
				logger.warning("ClientFrame: received message.");
			}
			catch (IOException e)
			{
			...
			{
				logger.warning("ClientFrame2: ClassNotFoundException");
			}
